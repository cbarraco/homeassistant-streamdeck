...
export class RunScriptPIAction extends ActionPI {
    ...
    override setUp(): void {
        super.setUp();
        logMessage("Injecting run script parameters");
    ...
    override update(homeAssistantCache: HomeAssistantCache): void {
        ...
        ActionPI.populateEntityOptions(entityIdSelector, "script", homeAssistantCache);
        if (this.settings.entityId) {
