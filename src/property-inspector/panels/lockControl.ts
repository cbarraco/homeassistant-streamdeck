...
const lockControlForm = createLockControlForm();

export class LockControlPIAction extends ActionPI {
    ...
    override setUp(): void {
        super.setUp();
        logMessage("Injecting lock control parameters");
        const controls = lockControlForm.ensureMounted();
        if (!controls) {
            return;
        }
        ...
        if (this.settings.lockCommand) {
            commandSelect.value = this.settings.lockCommand;
        }

        commandSelect.addEventListener("change", (event) => {
            const value = (event.target as HTMLSelectElement).value;
            this.settings.lockCommand = value;
            saveSettings(this.action, this.uuid, this.settings);
        });
    }

    override update(homeAssistantCache: HomeAssistantCache): void {
        const controls = lockControlForm.getControls();
        if (!controls) {
            return;
        }

        const { entitySelect } = controls;
        ActionPI.populateEntityOptions(entitySelect, "lock", homeAssistantCache);
...
function createLockControlForm(): {
