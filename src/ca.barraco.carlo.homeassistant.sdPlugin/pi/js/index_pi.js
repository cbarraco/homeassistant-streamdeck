...
$SD.on("piGlobalDataChanged", (returnValue) => {
    logStreamDeckEvent(returnValue);

    saveGlobalSettings(returnValue);
    sendValueToPlugin(returnValue, "sdpi_collection");
});
...
function saveGlobalSettings(sdpi_collection) {
    if (typeof sdpi_collection !== "object") return;
...
function gotCallbackFromWindow(parameter) {
    logStreamDeckEvent(parameter);
    $SD.emit("piGlobalDataChanged", parameter);
}
